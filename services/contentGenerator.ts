        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        copies: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { headline: { type: Type.STRING }, body: { type: Type.STRING }, cta: { type: Type.STRING }, hashtags: { type: Type.STRING } }, required: ['headline', 'body', 'cta'] } },
                    },
                    required: ['copies'],
                },
            },
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: { headline: { type: Type.STRING }, body: { type: Type.STRING }, cta: { type: Type.STRING }, hashtags: { type: Type.STRING } },
        ...
        const text = response.text?.trim();
        if (!text) throw new Error("AI returned an empty response for rewrite.");
