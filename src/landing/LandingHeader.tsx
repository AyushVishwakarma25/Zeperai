...
export const LandingHeader: React.FC = () => {
  ...
  return (
    <>
      <div className={`sticky top-2 md:top-6 z-50 px-4 max-w-6xl mx-auto transition-transform duration-500 ease-in-out ${isHeaderVisible ? 'translate-y-0 opacity-100' : '-translate-y-[150%] opacity-0'}`}>
        <nav className="bg-white/80 backdrop-blur-xl border border-slate-200/80 shadow-xl shadow-black/5 rounded-full px-4 py-2 md:px-6 md:py-2.5 relative z-50">
          <div className="flex justify-between items-center gap-2">
            <div className="flex items-center cursor-pointer shrink-0" onClick={() => navigate('/')}>
              <BrandLogo variant="full" color="black" className="w-24 md:w-28 h-auto" />
            </div>
            
            <div className="hidden lg:flex items-center space-x-5 text-sm font-medium text-slate-700">
              <Link 
                to="/#features" 
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-16 left-4 right-4 bg-white border border-slate-200 shadow-2xl rounded-2xl p-4 flex flex-col gap-2 text-center z-40 lg:hidden animate-fade-in">
            <Link 
              to="/#features" 
              onClick={(e) => handleScrollTo(e, 'features')}
