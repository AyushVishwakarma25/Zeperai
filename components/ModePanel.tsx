...
import { AspectRatio, ModelGender, ModelPersona, OutfitChoice, StylePreset, SkinTone, ClothingType, ModelChoice, AppMode, OutputFormat, AdLayout, ResolutionQuality, ProductCategory } from '../types';
import { 
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="mb-4 p-4 bg-white border border-border-light rounded-2xl">
...
export const ModePanel: React.FC<ModePanelProps> = ({ 
  ...
  const [customBackdropAndProps, setCustomBackdropAndProps] = useState<string>('');
  const [customTextPlacement, setCustomTextPlacement] = useState<string>('');
  ...
  const productStylePresetOptions = () => {
    if (!Array.isArray(PRO_PRODUCT_STYLE_PRESETS)) {
  ...
  return (
    <aside className="fixed top-0 right-0 bg-sidebar-light text-text-primary flex flex-col h-full w-[26rem] border-l border-border-light shadow-2xl z-40 animate-slide-in-from-right">
          ...
          <>
            ...
            {(mode === 'Influencer' || mode === AppMode.Fashion) && (
              <Section title="Influencer Details">
                ...
                <div className="relative mt-4">
                 ...
                 <OptionGroup label="Outfit Type" value={clothingType} onChange={(val) => handleParamChange('clothingType', val)} className="mt-4">
                    {CLOTHING_TYPE_OPTIONS.map(opt => (
                        <OptionButton key={opt.value} value={opt.value}><Icon name={opt.icon} className="w-5 h-5 mb-1"/>{opt.label}</OptionButton>
                    ))}
                  </OptionGroup>

                <Select label="Background Style" value={backgroundStyle} onChange={e => handleParamChange('backgroundStyle', e.target.value)} className="mt-4">
                    {backgroundOptions}
                </Select>
              </Section>
            )}

            {(mode === 'Product' || mode === AppMode.Amazon) && (
                <Section title="Photoshoot Style">
                    {isStoryboardMode ? (
                        <FormTextArea
                    ...
                    <Select label="Visual Style Preset" value={productStylePreset} onChange={e => handleParamChange('productStylePreset', e.target.value)} className="mt-4">
            ...
            {(mode === 'AdCreative' || mode === AppMode.Youtube || mode === AppMode.Banner) && (
              <Section title="Ad Creative Details">
                ...
                />
                 <OptionGroup label="Ad Layout" value={adLayout} onChange={(val) => handleParamChange('adLayout', val)} className="mt-4">
                 ...
                 <FormInput label="Title / Headline" id="ad-title" value={params.adTitle} onChange={e => handleParamChange('adTitle', e.target.value)} className="mt-4" />
        ...
        <Section title="Output Settings">
            <OptionGroup label="Aspect Ratio" value={aspectRatio} onChange={(val) => handleParamChange('aspectRatio', val)}>
             ...
             <div className="flex space-x-2 mt-4">
                <Select label="Format" value={outputFormat} onChange={e => handleParamChange('outputFormat', e.target.value)} className="flex-1">
                 ...
                 <Select label="Quality" value={params.resolutionQuality} onChange={e => handleParamChange('resolutionQuality', e.target.value)} className="flex-1">
       ...
       {isGenerateButtonReady && (
            <div className="p-4 border-t border-border-light">
                <Button 
                    ...
                    disabled={isLoading} 
                    className="!py-3 !text-base !rounded-xl shadow-lg w-full"
                >
